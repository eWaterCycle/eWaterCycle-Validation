...
GRDCLocation=../../localData/GRDC/2stationsDaily/
...
echo python ../GRDC2NetCDF/GRDC2NetCDF.py ${GRDCLocation} daily ${tmpFolder}/testDailyOld.nc 1990-01-01 2000-01-01

#THIS IS ONLY BECAUSE I DO NOT HAVE THE GRDC DATA YET AND MAKES NO HYDROLOGICAL SENSE
echo cdo shifttime,20year ${tmpFolder}/testDailyOld.nc ${tmpFolder}/testDaily.nc

#select one forecast-day. doing in this 2 steps, so this can be put into a loop later on.
echo cdo seldate,${targetDay},${endForcastDay} ${tmpFolder}/testDaily.nc ${tmpFolder}/GRDCSelection.nc

#interpolate output eWaterCycle (a grid) to GRDC station locations (points)
for ensembleMember in {01..05..1}
do
    echo cdo remapnn,${tmpFolder}/testDaily.nc ${eWaterOutput}/work${ensembleMember}/output/netcdf/discharge_dailyTot_output.nc ${tmpFolder}/eWaterCycleGRDCMap${ensembleMember}.nc
done
